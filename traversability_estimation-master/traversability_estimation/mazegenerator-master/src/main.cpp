...
void usage(std::ostream &out) {
  out << "Usage: mazegen [--help] [-m <maze type>] [-a <algorithm type>]"
      << std::endl;
  out << "               [-s <size> | -w <width> -h <height>] -n <number>]" << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  if (optionmap["-t"] == 0) {
	  std::cout << "Rendering maze to '" << outputprefix << std::to_string(optionmap["-n"]) << ".svg'..."
              << std::endl;
    maze->PrintMazeSVG(outputprefix + std::to_string(optionmap["-n"]));
  } else {
