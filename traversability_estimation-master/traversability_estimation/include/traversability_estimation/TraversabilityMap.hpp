...
class TraversabilityMap
{
 ...
 public:
   ...
   * @return true if successful.
   */
  bool traversabilityFootprint(double footprintIntervallYaw);

  void saveTravImage(const std::string& target, const std::string& name);
  ...
  bool setupTraverabilityMap(double footprintIntervallYaw);

  std::vector<double> traversabilityAtPosition(const Eigen::Array2i& index);
 ...
 private:
  ...
  //! variables for traversabilityAtPosition
  std::vector<grid_map::Polygon> polygons;
  std::vector<Eigen::Quaterniond> orientations;

  struct traverabilityMem {
	grid_map::Position gridMapPosition;
    std::vector<double> traversabilitys;
  };
  std::vector<traverabilityMem> traverabilityMap;
