...
void Maze::PrintMazeSVG(const std::string& outputprefix) const {
  ...
  std::ofstream myfile;
  myfile.open (outputprefix + ".txt");
  double xmin, ymin, xmax, ymax;
  std::tie(xmin, ymin, xmax, ymax) = GetCoordinateBounds();
  int xresolution = (xmax - xmin + 2) * 30,
      yresolution = (ymax - ymin + 2) * 30;
  myfile <<  "xmax " + std::to_string(xmax) + " xmin " + std::to_string(xmin) + " ymax " + std::to_string(ymax) + " ymin " + std::to_string(ymin) + "\n";
  myfile <<  "xresolution " + std::to_string(xresolution) + " xresolution " + std::to_string(xresolution) + "\n";
