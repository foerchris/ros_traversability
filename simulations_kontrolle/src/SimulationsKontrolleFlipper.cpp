/*
 * SimulationsKontrolleFlipper.cpp
 *
 *  Created on: 04.07.2019
 *      Author: chfo
 */

#include "simulations_kontrolle/SimulationsKontrolleFlipper.h"
...
SimulationsKontrolleFlipper::SimulationsKontrolleFlipper(ros::NodeHandle& nodeHandle)
		  ...
		  getObjectInfoFromYaml_(nodeHandle)
{
	...
	//resetRobot = nodeHandle_.advertiseService("reset_robot", &SimulationsKontrolleFlipper::resetRobotSrv, this);
	resetRobot = nodeHandle_.advertiseService("/" + tf_prefix+"/reset_robot", &SimulationsKontrolleFlipper::resetRobotSrv, this);
	...
	//it_sub = it.subscribe("elevation_map_image", 1, boost::bind (&SimulationsKontrolleFlipper::MapImageCallback, this, _1));
	it_sub = it.subscribe("/" + tf_prefix+"/elevation_map_image", 1, boost::bind (&SimulationsKontrolleFlipper::MapImageCallback, this, _1));

	msg_timer = nodeHandle_.createTimer(ros::Duration(0.1), boost::bind (&SimulationsKontrolleFlipper::publischGoal, this, _1));
	reset_timer = nodeHandle_.createTimer(ros::Duration(0.1), boost::bind (&SimulationsKontrolleFlipper::resetCallback, this, _1));
...
SimulationsKontrolleFlipper::~SimulationsKontrolleFlipper ()
{
	destroyWorld();
}

void SimulationsKontrolleFlipper::resetCallback(const ros::TimerEvent& event)
{
	creatEnviroment();
}


void SimulationsKontrolleFlipper::creatEnviroment()
{
	if(resetWorld)
	{
...
void SimulationsKontrolleFlipper::clcGoalPathSrvsCall()
{
...
void SimulationsKontrolleFlipper::publischGoal(const ros::TimerEvent& bla)
//void SimulationsKontrolleFlipper::publischGoal(const geometry_msgs::Pose& goalPose)
{
...
void SimulationsKontrolleFlipper::setObject(const string& modelName, geometry_msgs::Pose startPose)
{
...
void SimulationsKontrolleFlipper::destroyWorld()
{
...
void SimulationsKontrolleFlipper::generateWorld(int minObjects, int maxObjects)
{
...
void SimulationsKontrolleFlipper::generateWorld2()
{
...
void SimulationsKontrolleFlipper::setObjectInWorld()
{
	random_device rd;
	mt19937 mt(rd());
...
geometry_msgs::Pose SimulationsKontrolleFlipper::setRandomObst(const object_options& objectOptions,const bool& mirror, const double& lastX)
{
	geometry_msgs::Pose pose;
...
void SimulationsKontrolleFlipper::resetAllObjects()
{
	geometry_msgs::Pose pose;
	pose.position.x = 0;
	pose.position.y = 10;
...
double SimulationsKontrolleFlipper::creatRndPosition(const min_max_object_pose& minMaxObjectPose)
{
	random_device rd;
	mt19937 mt(rd());
	double value;
...
void SimulationsKontrolleFlipper::spwanObject(const string& modelName, const string& xmlName, geometry_msgs::Pose startPose)
{
	startPose = tfTransform(startPose, ODOM_FRAME, MAP_FRAME);
...
void SimulationsKontrolleFlipper::deleteObject(const string& modelName)
{
...
string  SimulationsKontrolleFlipper::readXmlFile(const string& name)
{
...
void SimulationsKontrolleFlipper::poseToOdomMsg(const geometry_msgs::Pose& pose, nav_msgs::Odometry& setPose)
{
...
visualization_msgs::Marker SimulationsKontrolleFlipper::createMarker (std::string ns, int id, double x, double y,  double r = 1.0, double g = 0.0, double b = 0.0, double a = 1.0)
{
...
bool SimulationsKontrolleFlipper::resetRobotSrv(std_srvs::Empty::Request &req,
		std_srvs::Empty::Response &res)
{
	setRobotZeroPose();
	return true;
}
void SimulationsKontrolleFlipper::setRobotZeroPose()
{
...
geometry_msgs::Pose SimulationsKontrolleFlipper::tfTransform(const geometry_msgs::Pose& pose,const string& destination_frame,const string& original_frame)
{
...
void SimulationsKontrolleFlipper::MapImageCallback(const sensor_msgs::ImageConstPtr& msg)
{
